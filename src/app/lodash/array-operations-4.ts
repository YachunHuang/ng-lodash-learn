...
@Component({
  selector: 'array-operations-4',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './array-operations.html',
})
export class ArrayOperations4Component {
  ...
  examples = [
    {
      ...
      tip: '就像是把不同的拼圖塊合併成一個大拼圖，沒有改變原本的拼圖，只是把它們湊在一起。',
      demo: ''
    },
    {
    ...
    },
    {
      ...
      desc: '將陣列展開到指定的層數，讓你可以控制要展開幾層的巢狀陣列，超過的層數就不會展開。',
...
Level 4: 4
    ...
    },
    {
      ...
      tip: '給你一個由「對偶」組成的陣列（每個元素是一個 [key, value] 的組合）。會把它轉換成物件，key、value 就變成物件的屬性和值。',
      demo: ''
    },
    {
      title: '_.join(array, [separator=\',\'])',
      desc: '將陣列中的元素用指定的分隔符連接成一個字符串。',
      code: '_.join([\'a\', \'b\', \'c\'], \'-\')',
      result: _.join(['a', 'b', 'c'], '-'),
      tip: '就是把數字、字母，甚至是物件元素都「串起來」形成一個句子。可以自訂分隔符（例如，逗號、空格或其他符號）。',
      demo: ''
    },
  ];
}
